...
export const HomeView: React.FC<Props> = ({ onNavigate, onOpenMutation }) => {
  ...
  return (
    <div className="space-y-16 pb-16">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-8 space-y-6">
            ...
            <h1 className="text-3xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-white font-display leading-[1.1]">
              Descubre el fascinante mundo del <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#74C69D] to-[#B7E4C7]">periquito australiano</span>
            </h1>

            <p className="text-base sm:text-lg text-[#D8E2DC] max-w-2xl leading-relaxed">
              Aprende sobre genética, mutaciones, líneas de color, cuidados responsables, salud aviar y herramientas de registro para criadores y aficionados de <em>Melopsittacus undulatus</em>.
            </p>
      ...
      {/* 2. DUAL AUDIENCE PATHWAYS (Principiante vs Criador) */}
      ...
      {/* 3. CORE MODULE TILES (6 Key Modules) */}
        ...
        <div className="flex flex-col lg:flex-row items-center gap-8">
          <div className="w-full lg:w-1/2 space-y-4">
            ...
            <div className="p-4 rounded-xl bg-[#F1F3F2] border border-[#D8E2DC] text-xs sm:text-sm text-[#1B4332]">
              <span className="font-bold text-[#1B4332] block mb-1">Clave de Identificación:</span>
          ...
          <div className="w-full lg:w-1/2">
            <div className="relative rounded-2xl overflow-hidden shadow-md border border-[#D8E2DC] group">
              <img
                src={featuredMutation.images[0]?.url || 'https://images.unsplash.com/photo-1552728089-57bdde30beb3?auto=format&fit=crop&w=800&q=80'}
                alt={featuredMutation.name}
                className="w-full h-72 sm:h-80 object-cover group-hover:scale-105 transition-transform duration-500"
                referrerPolicy="no-referrer"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#1B4332]/90 via-transparent to-transparent flex items-end p-4">
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {COLOR_LINES_DATA.map((line) => (
            <div key={line.id} className="p-6 rounded-2xl bg-white border border-[#D8E2DC] shadow-xs space-y-4">
              <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-3 gap-2 pt-2">
                {line.darkFactorLevels.map((df) => (
                  <div key={df.factor} className="p-2.5 rounded-xl border border-[#D8E2DC] bg-[#F1F3F2] text-center">
                    <div
                      className="w-6 h-6 rounded-full mx-auto mb-1.5 shadow-xs border border-white"
                      style={{ backgroundColor: df.hexColor }}
                    />
                    <div className="text-xs font-bold text-[#1B4332] truncate" title={df.name}>
